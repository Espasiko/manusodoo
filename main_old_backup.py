...
async def get_products(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        # Usar ODOO_XMLRPC_URL si está definido, de lo contrario usar la URL normal
        xmlrpc_url = os.getenv('ODOO_XMLRPC_URL', odoo_config['url'])
        
        # Autenticación
        common = xmlrpc.client.ServerProxy(f'{xmlrpc_url}/xmlrpc/2/common')
        ...
        if not uid:
            ...
            return products  # Fallback a datos simulados
        
        # Conexión al modelo de productos
        models = xmlrpc.client.ServerProxy(f'{xmlrpc_url}/xmlrpc/2/object')
        ...
        if not product_ids:
            print("Advertencia: No se encontraron productos en Odoo")
            return products  # Fallback a datos simulados
